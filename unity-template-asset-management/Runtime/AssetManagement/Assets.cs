...
public class Asset : Singleton<Asset>
{
    ...
    public T Get<T>(string k) where T : MonoBehaviour
    {
        //repository.GetValueOrDefault(k)?.GetComponent<T>();
        //Debug.Log("Finding asset: " + k);
        return transform.Find(k).GetComponent<T>();
    }
